...
def process_sen_manual(dir_item:str, sen_votes:list[dict], overwrite:bool):
    java_file_path = os.path.join(dir_item, "code_file")
    ...
    for code in tqdm(sen_votes):
        ...
        current_file_path = os.path.join(java_file_path, f"{hash_index}_python")
        if not os.path.exists(current_file_path):
            os.makedirs(current_file_path)

        main_file = os.path.join(current_file_path, "main.py")
        # skip
...
def main(overwrite=False):
    ...
    for dir_item in dirs:
        ...
        sen_votes = read_code_block(os.path.join(directory_path,dir_item), "sen_manual")
        if len(sen_votes) == 0:
            shutil.rmtree(os.path.join(directory_path, dir_item))
            continue
        logger.info(f"This project have  sen_manual {len(sen_votes)} codes.")
        process_sen_manual(os.path.join(directory_path,dir_item), sen_votes , overwrite)
